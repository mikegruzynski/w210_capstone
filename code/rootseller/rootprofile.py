...
class UserProfile(object):
    def __init__(self, userid):
        ...
        self.init_macro = macronutrients.Macronutrients(self.userprofile_df)
        self.init_micro = micronutrients.MicroNutrients(self.userprofile_df)

        self.macro_label_list = eval(self.userprofile_df['macro_nutrients'].get_values()[0])
        self.macro_list = self.init_macro.convert_labels_to_df_columns(self.macro_label_list)

        self.micro_label_list = eval(self.userprofile_df['micro_nutrients'].get_values()[0])
        self.micro_list = self.init_micro.convert_labels_to_df_columns(self.micro_label_list)

        self.profile_macro_df = self.init_macro.macro_daily_macro_estimation()
        self.profile_micro_df = self.init_micro.micro_daily_macro_estimation()
