...
class Recipes(object):
    def __init__(self, profile_init):
        ...
        try:
            self.ambiguous_df = pd.read_csv('C:/Users/mgruz/Desktop/w210/data/recipe/ambiguous_conversion_use.csv')
        except:
            self.ambiguous_df = pd.read_csv('C:/Users/pa351d/Desktop/w210/data/recipe/ambiguous_conversion_use.csv')
    ...
    def extract_recipe_dataframe(self, tag, conversion_factor):
        ...
        df_temp = nutrtion_init.NDB_NO_lookup(tag, filter_list=self.filter_list)
        df_temp = df_temp * float(conversion_factor)
        df_temp_tag = nutrtion_init.NDB_NO_lookup(tag, filter_list=['NDB_NO', 'Description', 'Category'])

        return df_temp_tag.join(df_temp)

    def unit_food_conversion(self, recipe_amount, recipe_unit, tag, recipe_ingredient):
        nutrtion_init = nutrtion.Nutrition()
        temp_nutritional_df = nutrtion_init.NDB_NO_lookup(tag, filter_list=['Measure', 'Weight(g)'])
        ...
        nutrition_df_amount = float(filter(None, re.findall(r"([/.0-9]*)", nutrition_df_measure))[0])
        ...
        if recipe_type == 'weight':
            ...
            # conversion_factor = 'LOOKUP DATABASE'
    ...
    def extact_number_from_recipe(self, ingredient, food_unit_dict):
        ingredient_split = ingredient.split("-")
        ingredient_split = " ".join(ingredient_split)
        amount_list = filter(None, re.findall(r"([/.0-9]*)", ingredient_split))
        ...
        amount_key_list = food_amount_dict.keys()
        unit_key_list = food_unit_dict.keys()
    ...
    def recipe_list_to_conversion_factor_list(self, recipe_id):
